...
static QString _q_escapeIdentifier(const QString &identifier) {
  ...
  if (!identifier.isEmpty() && !identifier.startsWith(QLatin1Char('"')) &&
...
static QVariant::Type qGetColumnType(const QString &tpName) {
  ...
  if (typeName == QLatin1String("double") ||
      typeName == QLatin1String("float") || typeName == QLatin1String("real") ||
      typeName.startsWith(QLatin1String("numeric")))
    return QVariant::Double;
  if (typeName == QLatin1String("blob")) return QVariant::ByteArray;
...
static QSqlError qMakeError(sqlite3 *access, const QString &descr,
  ...
  return QSqlError(
      descr, QString(reinterpret_cast<const QChar *>(sqlite3_errmsg16(access))),
...
class QSQLiteResultPrivate : public QSqlCachedResultPrivate {
 ...
 public:
  ...
  void cleanup();
  bool fetchNext(QSqlCachedResult::ValueCache &values, int idx,
                 bool initialFetch);
...
QSQLiteResultPrivate::QSQLiteResultPrivate(QSQLiteResult *q,
                                           const QSQLiteDriver *drv)
...
void QSQLiteResultPrivate::initColumns(bool emptyResultset) {
  ...
  for (int i = 0; i < nCols; ++i) {
    QString colName =
        QString(reinterpret_cast<const QChar *>(sqlite3_column_name16(stmt, i)))
            .remove(QLatin1Char('"'));
    const QString tableName = QString(reinterpret_cast<const QChar *>(
                                          sqlite3_column_table_name16(stmt, i)))
                                  .remove(QLatin1Char('"'));
    // const QString tableName =
    //     QString(reinterpret_cast<const QChar *>(sqlite3_column_name16(stmt,
    //     i)))
    //         .remove(QLatin1Char('"'));
    // must use typeName for resolving the type to match QSqliteDriver::record
    QString typeName = QString(
        reinterpret_cast<const QChar *>(sqlite3_column_decltype16(stmt, i)));
...
bool QSQLiteResultPrivate::fetchNext(QSqlCachedResult::ValueCache &values,
  ...
  if (skipRow) {
    ...
    skipRow = false;
    for (int i = 0; i < firstRow.count(); i++) values[i] = firstRow[i];
  ...
  switch (res) {
    case SQLITE_ROW:
      ...
      if (rInf.isEmpty())
        // must be first call.
        initColumns(false);
      if (idx < 0 && !initialFetch) return true;
        ...
        switch (sqlite3_column_type(stmt, i)) {
          ...
          default:
            values[i + idx] = QString(
                reinterpret_cast<const QChar *>(sqlite3_column_text16(stmt, i)),
...
QSQLiteResult::~QSQLiteResult() {
  Q_D(QSQLiteResult);
  if (d->drv_d_func())
    const_cast<QSQLiteDriverPrivate *>(d->drv_d_func())
        ->results.removeOne(this);
  d->cleanup();
}
...
bool QSQLiteResult::prepare(const QString &query) {
  Q_D(QSQLiteResult);
  if (!driver() || !driver()->isOpen() || driver()->isOpenError()) return false;
  ...
  int res = sqlite3_prepare16_v2(d->drv_d_func()->access, query.constData(),
                                 (query.size() + 1) * sizeof(QChar), &d->stmt,
                                 &pzTail);
#else
  int res =
      sqlite3_prepare16(d->access, query.constData(),
                        (query.size() + 1) * sizeof(QChar), &d->stmt, &pzTail);
#endif

  if (res != SQLITE_OK) {
    setLastError(qMakeError(d->drv_d_func()->access,
                            QCoreApplication::translate(
                                "QSQLiteResult", "Unable to execute statement"),
                            QSqlError::StatementError, res));
    d->finalize();
    return false;
  } else if (pzTail && !QString(reinterpret_cast<const QChar *>(pzTail))
                            .trimmed()
                            .isEmpty()) {
    setLastError(qMakeError(
        d->drv_d_func()->access,
        QCoreApplication::translate(
            "QSQLiteResult", "Unable to execute multiple statements at a time"),
...
bool QSQLiteResult::execBatch(bool arrayBind) {
  ...
  for (int i = 0; i < values.at(0).toList().count(); ++i) {
    ...
    while (it != d->indexes.constEnd()) {
      bindValue(it.key(), values.at(it.value().first()).toList().at(i),
                QSql::In);
      ++it;
    }
...
bool QSQLiteResult::exec() {
  ...
  if (res != SQLITE_OK) {
    setLastError(qMakeError(d->drv_d_func()->access,
                            QCoreApplication::translate(
                                "QSQLiteResult", "Unable to reset statement"),
                            QSqlError::StatementError, res));
    d->finalize();
    return false;
  }
  ...
  // We need to check explicitly that paramCount is greater than or equal to 1,
  // as sqlite can end up in a case where for virtual tables it returns 0 even
  // though it has parameters
  if (paramCount >= 1 && paramCount < values.count()) {
    const auto countIndexes = [](int counter, const QVector<int> &indexList) {
      return counter + indexList.length();
    };

    const int bindParamCount = std::accumulate(
        d->indexes.cbegin(), d->indexes.cend(), 0, countIndexes);
    ...
    for (int i = 0, currentIndex = 0; i < values.size(); ++i) {
      ...
      const auto placeHolder = QString::fromUtf8(
          sqlite3_bind_parameter_name(d->stmt, currentIndex + 1));
  ...
  if (paramCountIsValid) {
      ...
      if (value.isNull()) {
        ...
        switch (value.type()) {
          case QVariant::ByteArray: {
            ...
            res = sqlite3_bind_blob(d->stmt, i + 1, ba->constData(), ba->size(),
                                    SQLITE_STATIC);
            break;
          }
          case QVariant::Int:
    ...
    setLastError(QSqlError(QCoreApplication::translate(
                               "QSQLiteResult", "Parameter count mismatch"),
...
int QSQLiteResult::size() { return -1; }
...
QSqlRecord QSQLiteResult::record() const {
  Q_D(const QSQLiteResult);
  if (!isActive() || !isSelect()) return QSqlRecord();
  return d->rInf;
}

void QSQLiteResult::detachFromResultSet() {
  Q_D(QSQLiteResult);
  if (d->stmt) sqlite3_reset(d->stmt);
}
...
#if QT_CONFIG(regularexpression)
static void _q_regexp(sqlite3_context *context, int argc,
                      sqlite3_value **argv) {
  ...
  auto cache = static_cast<QCache<QString, QRegularExpression> *>(
  ...
  if (!wasCached) cache->insert(pattern, regexp);
...
QSQLiteDriver::~QSQLiteDriver() { close(); }
...
bool QSQLiteDriver::open(const QString &db, const QString &, const QString &,
...
#if QT_CONFIG(regularexpression)
  static const QLatin1String regexpConnectOption =
      QLatin1String("QSQLITE_ENABLE_REGEXP");
  ...
  for (auto option : opts) {
    ...
    if (option.startsWith(QLatin1String("QSQLITE_BUSY_TIMEOUT"))) {
      ...
      if (option.isEmpty()) {
        ...
        if (ok) {
          defineRegexp = true;
          if (cacheSize > 0) regexpCacheSize = cacheSize;
        }
      }
    }
#endif
  ...
  int openMode =
      (openReadOnlyOption ? SQLITE_OPEN_READONLY
                          : (SQLITE_OPEN_READWRITE | SQLITE_OPEN_CREATE));
  openMode |=
      (sharedCache ? SQLITE_OPEN_SHAREDCACHE : SQLITE_OPEN_PRIVATECACHE);
  if (openUriOption) openMode |= SQLITE_OPEN_URI;

  openMode |= SQLITE_OPEN_NOMUTEX;

  const int res =
      sqlite3_open_v2(db.toUtf8().constData(), &d->access, openMode, NULL);
  ...
  if (res == SQLITE_OK) {
    ...
    if (defineRegexp) {
      ...
      sqlite3_create_function_v2(d->access, "regexp", 2, SQLITE_UTF8, cache,
                                 &_q_regexp, NULL, NULL, &_q_regexp_cleanup);
    }
#endif
    return true;
  } else {
...
bool QSQLiteDriver::beginTransaction() {
  if (!isOpen() || isOpenError()) return false;
...
static QSqlIndex qGetTableInfo(QSqlQuery &q, const QString &tableName,
...
static void handle_sqlite_callback(void *qobj, int aoperation,
                                   char const *adbname, char const *atablename,
  ...
  if (driver) {
    QMetaObject::invokeMethod(
        driver, "handleNotification", Qt::QueuedConnection,
...
bool QSQLiteDriver::subscribeToNotification(const QString &name) {
  ...
  // sqlite supports only one notification callback, so only the first is
  // registered
  d->notificationid << name;
  if (d->notificationid.count() == 1)
    sqlite3_update_hook(d->access, &handle_sqlite_callback,
...
bool QSQLiteDriver::unsubscribeFromNotification(const QString &name) {
  ...
  if (d->notificationid.isEmpty()) sqlite3_update_hook(d->access, NULL, NULL);
